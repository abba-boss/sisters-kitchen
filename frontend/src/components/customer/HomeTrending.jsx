...
import { ChevronRight, ChevronLeft, Star, Clock, Heart, ShoppingCart, Flame, Award, Zap, CheckCircle } from 'lucide-react';
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="font-poppins font-bold text-2xl text-brand-dark">Trending Today</h2>
              <Flame size={20} className="text-primary" />
            </div>
            <p className="text-sm text-brand-muted mt-1">Popular food content and top-selling dishes customers are ordering now.</p>
          </div>
          <div className="flex items-center gap-2">
            <div className="hidden md:flex items-center gap-2">
              <button onClick={() => scroll(-1)} className="w-10 h-10 rounded-full border border-orange-100 bg-white text-brand-muted hover:text-primary hover:border-primary/30 transition-colors flex items-center justify-center">
                <ChevronLeft size={16} />
              </button>
              <button onClick={() => scroll(1)} className="w-10 h-10 rounded-full border border-orange-100 bg-white text-brand-muted hover:text-primary hover:border-primary/30 transition-colors flex items-center justify-center">
        ...
        {loading ? (
          <div className="flex gap-4 overflow-hidden">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="flex-shrink-0 w-[18.5rem] skeleton rounded-[1.75rem] h-[22rem]" />
            ))}
          </div>
        ) : (
          <div
    ...
    >
        ...
        <div className="absolute left-3 right-3 bottom-3">
          <div className="inline-flex items-center gap-2 rounded-full bg-white/82 backdrop-blur-md px-2.5 py-1 shadow-soft">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-brand-muted">Fast local delivery</p>
            </div>
          </div>
        </div>
        {/* Wishlist */}
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs text-primary font-semibold mb-0.5 truncate">Trending Pick</p>
            <h3 className="font-semibold text-brand-dark text-[15px] mb-2 line-clamp-1">{product.name}</h3>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <span className="font-poppins font-bold text-brand-dark text-lg">{formatPrice(price)}</span>
            {product.discountPrice && (
