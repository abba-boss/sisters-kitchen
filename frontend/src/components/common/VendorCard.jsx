      ...
      <Link to={`/vendors/${vendor.id}`}>
        ...
        <div className="relative h-36 overflow-hidden">
          ...
          <div className={`absolute top-3 right-3 flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${
            ...
            <span className={`w-1.5 h-1.5 rounded-full ${vendor.isOpen ? 'bg-white' : 'bg-brand-muted'}`} />
        ...
        <div className="p-4 -mt-8 relative">
          ...
          {vendor.description && (
            <p className="text-xs text-brand-muted line-clamp-2 mb-3">{vendor.description}</p>
          )}

          <div className="flex items-center justify-between text-xs text-brand-muted">
            <div className="flex items-center gap-1">
              <Star size={12} fill="#FF7A59" className="text-primary" />
              <span className="font-semibold text-brand-dark">{vendor.rating?.toFixed(1) || '—'}</span>
