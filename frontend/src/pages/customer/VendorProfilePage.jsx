...
export default function VendorProfilePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      followerService
        .getStatus(id)
        .catch(() => ({ data: { data: { followersCount: 0 } } })),
    ])
      .then(([vRes, rRes, pRes, sRes, fRes]) => {
        console.log("API responses:", { vRes, rRes, pRes, sRes, fRes });
    ...
    return (
      <MainLayout>
        <div className="page-container py-20 text-center">
          ...
          <Link to="/vendors" className="btn-primary mt-4 inline-block">
            Browse Vendors
          </Link>
        </div>
      </MainLayout>
  ...
  const heroMeta = [
    { icon: Users, label: "Followers", value: followersCount },
    {
      icon: UserRoundPlus,
      label: "Following",
      value: Math.max(6, Math.round((posts.length || 1) * 1.5)),
    },
  ...
  const highlightItems = [
    ...
    { label: "Reviews", icon: Star, count: reviews.length },
    {
      ...
      icon: Sparkles,
      count: posts.filter((p) => p.type === "behind_scenes").length,
    },
    {
      label: "Offers",
      icon: Gift,
      count: posts.filter((p) => p.type === "promotion").length,
    },
    {
      label: "Recipes",
      icon: BookOpen,
      count: posts.filter((p) => p.type === "recipe").length,
    },
  ];
                  ...
                  <section>
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        ...
                        <p className="text-sm text-brand-muted">
                          Cooking updates, specials, and behind-the-scenes
                          moments.
                        </p>
                      </div>
                    </div>
                    {posts.length === 0 ? (
                      <EmptyState
                        icon={MessageSquare}
                        title="No posts yet"
                        message="This vendor hasn't posted anything yet."
                      />
                    ) : (
                  ...
                  <section>
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        ...
                        <p className="text-sm text-brand-muted">
                          Short cooking clips and visual kitchen updates.
                        </p>
                      </div>
                    </div>
                    {videoPosts.length === 0 ? (
                      <EmptyState
                        icon={Camera}
                        title="No videos yet"
                        message="This vendor hasn't shared any videos yet."
                      />
                    ) : (
                      <div className="max-w-3xl space-y-5">
                        {videoPosts.map((post) => (
