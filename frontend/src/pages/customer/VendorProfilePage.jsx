...
import { PageLoader, ProductSkeleton, VendorSkeleton } from '../../components/common/LoadingSkeleton';
...
export default function VendorProfilePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      postService.getVendorPosts(id, { limit: 12 }),
      storyService.getVendorStories(id).catch(() => ({ data: { data: [] } })),
      followerService.getStatus(id).catch(() => ({ data: { data: { followersCount: 0 } } })),
    ])
      .then(([vRes, rRes, pRes, sRes, fRes]) => {
        setVendor(vRes.data.data);
        setReviews(rRes.data.data || []);
        setPosts(pRes.data.data || []);
        setStories(sRes.data.data || []);
        setFollowersCount(fRes.data.data?.followersCount || 0);
      })
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([vRes, pRes]) => {
        setSuggestedVendors((vRes.data.data || []).filter((v) => v.id !== id).slice(0, 4));
  ...
  const cover = vendor.coverImage || 'https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=1400';
  const products = vendor.products || [];
  const availableProducts = vendor.isOpen ? products.filter((p) => p.isAvailable) : [];
  const videoPosts = posts.filter((post) => post.media?.some((m) => m.type === 'video'));
  const filteredReviews = reviewFilter ? reviews.filter((rv) => rv.rating === reviewFilter) : reviews;
        ...
        <div className="page-container relative -mt-24 sm:-mt-28 lg:-mt-32 pb-10">
          <div className="grid xl:grid-cols-[minmax(0,1fr)_320px] gap-8 items-start">
              ...
              <div className="rounded-[2rem] bg-white/88 backdrop-blur-2xl border border-white/70 shadow-[0_20px_60px_rgba(74,44,42,0.12)] p-5 sm:p-7">
                <div className="flex flex-col lg:flex-row lg:items-end gap-5">
                  <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-[2rem] border-4 border-white shadow-card-hover overflow-hidden bg-white flex-shrink-0">
                    {vendor.logo ? (
                      ...
                      <div className="w-full h-full bg-primary/10 flex items-center justify-center">
                        <span className="font-poppins font-bold text-primary text-4xl">{vendor.businessName?.[0]}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      <h1 className="font-poppins font-bold text-3xl sm:text-4xl text-brand-dark">{vendor.businessName}</h1>
                      ...
                      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${
                        vendor.isOpen ? 'bg-accent text-white' : 'bg-gray-100 text-gray-500'
                      }`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${vendor.isOpen ? 'bg-white animate-pulse' : 'bg-gray-400'}`} />
                        {vendor.isOpen ? 'Open Now' : 'Closed'}
                      </span>
                    </div>

                    <p className="text-brand-muted text-sm sm:text-base max-w-3xl leading-relaxed">
                      {vendor.description || 'Homemade meals, fresh specials, social stories, and premium food experiences from a trusted local kitchen.'}
                    </p>

                    <div className="flex flex-wrap gap-4 mt-4 text-sm text-brand-muted">
                      {vendor.address && <div className="flex items-center gap-1.5"><MapPin size={14} className="text-primary" />{vendor.address}</div>}
                      {vendor.phone && <div className="flex items-center gap-1.5"><Phone size={14} className="text-primary" />{vendor.phone}</div>}
                      {vendor.openingTime && vendor.closingTime && (
                        <div className="flex items-center gap-1.5"><Clock size={14} className="text-primary" />{vendor.openingTime} - {vendor.closingTime}</div>
                      )}
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3 mt-6">
                  {heroMeta.map(({ icon: Icon, label, value }) => (
                    <div key={label} className="rounded-2xl bg-brand-bg/80 border border-orange-100 px-3 py-3">
                      <div className="w-8 h-8 rounded-xl bg-white text-primary flex items-center justify-center shadow-soft mb-2">
                        <Icon size={15} />
                      </div>
                      <p className="font-poppins font-bold text-brand-dark text-lg truncate">{value}</p>
              ...
              <section className="mt-6">
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                  {quickStats.map(({ label, value, icon: Icon }) => (
                    <div key={label} className="rounded-[1.7rem] bg-white border border-orange-100 shadow-card p-4">
                      <div className="w-10 h-10 rounded-2xl bg-primary/10 text-primary flex items-center justify-center mb-3">
                        <Icon size={18} />
                      </div>
                      <p className="font-poppins font-bold text-2xl text-brand-dark">{value}</p>
              ...
              <div className="mt-6 space-y-8">
                  ...
                  <>
                    <section>
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <h2 className="font-poppins font-bold text-2xl text-brand-dark">Menu</h2>
                          <p className="text-sm text-brand-muted">Fresh dishes ready for ordering.</p>
                        </div>
                      </div>

                      {availableProducts.length === 0 ? (
                        <EmptyState icon={ShoppingBag} title="No products yet" message="This vendor hasn't added products." />
                      ) : (
                    ...
                    <section>
                      ...
                      {upcomingMeals.length === 0 ? (
                        <EmptyState icon={CalendarClock} title="No upcoming meals yet" message="Check back later for scheduled meal drops." />
                      ) : (
                        <div className="space-y-4">
                          {upcomingMeals.map((meal, i) => (
                            ...
                            >
                              <img
                                src={meal.images?.[0] || 'https://images.unsplash.com/photo-1565299624946-b28f40a0ae38?w=320'}
                                alt={meal.name}
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <h3 className="font-poppins font-bold text-lg text-brand-dark">{meal.name}</h3>
                ...
                {tab === 'posts' && (
                  <section>
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h2 className="font-poppins font-bold text-2xl text-brand-dark">Social Posts</h2>
                        <p className="text-sm text-brand-muted">Cooking updates, specials, and behind-the-scenes moments.</p>
                      </div>
                    </div>
                    {posts.length === 0 ? (
                      <EmptyState icon={MessageSquare} title="No posts yet" message="This vendor hasn't posted anything yet." />
                    ) : (
                      <div className="max-w-3xl space-y-5">
                        {posts.map((post) => (
                          <PostCard key={post.id} post={post} showVendor={false} />
                        ))}
                      </div>
                    )}
                  </section>
                ...
                {tab === 'videos' && (
                  <section>
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h2 className="font-poppins font-bold text-2xl text-brand-dark">Videos</h2>
                        <p className="text-sm text-brand-muted">Short cooking clips and visual kitchen updates.</p>
                      </div>
                    </div>
                    {videoPosts.length === 0 ? (
                      <EmptyState icon={Camera} title="No videos yet" message="This vendor hasn't shared any videos yet." />
                    ) : (
                      <div className="max-w-3xl space-y-5">
                        {videoPosts.map((post) => (
                          <PostCard key={post.id} post={post} showVendor={false} />
                  ...
                  <section className="grid xl:grid-cols-[320px_minmax(0,1fr)] gap-6">
                    <div className="rounded-[1.8rem] bg-white border border-orange-100 shadow-card p-6 h-fit">
                      <div className="text-center mb-6">
                        ...
                        <p className="text-sm text-brand-muted mt-2">{reviews.length} reviews</p>
                      </div>
                    ...
                    <div className="space-y-4">
                      ...
                      {isAuthenticated && (
                        <form onSubmit={handleReview} className="rounded-[1.8rem] bg-white border border-orange-100 shadow-card p-5">
                  ...
                  <section className="grid xl:grid-cols-[minmax(0,1fr)_320px] gap-6">
                    <div className="space-y-6">
                      <div className="rounded-[1.8rem] bg-white border border-orange-100 shadow-card p-6">
                        <h2 className="font-poppins font-bold text-2xl text-brand-dark mb-3">Business Story</h2>
                        <p className="text-brand-muted leading-relaxed text-sm sm:text-base">
                          {vendor.description || 'This kitchen is built around homemade meals, seasonal specials, and customer-first service. Every order is prepared with care and shared through a premium social storefront experience.'}
                        </p>
                      </div>

                      <div className="rounded-[1.8rem] bg-white border border-orange-100 shadow-card p-6">
                        <h2 className="font-poppins font-bold text-2xl text-brand-dark mb-4">Kitchen Photos</h2>
                        <div className="grid sm:grid-cols-3 gap-3">
                          {[cover, vendor.logo || cover, stories[0]?.mediaUrl || cover].map((image, index) => (
                            <img
                              key={index}
                              src={image}
                              alt=""
                      ...
                      <div className="rounded-[1.8rem] bg-white border border-orange-100 shadow-card p-6">
                        <h2 className="font-poppins font-bold text-2xl text-brand-dark mb-4">Opening Hours</h2>
                          ...
                          {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map((day) => (
                            ...
                            >
                              ...
                              <p className="text-xs mt-1">
                                {vendor.availableDays?.includes(day) ? `${vendor.openingTime || '--'}-${vendor.closingTime || '--'}` : 'Closed'}
                              </p>
                            </div>
                          ))}
                        </div>
                    ...
                    <div className="space-y-4">
                      <div className="rounded-[1.8rem] bg-white border border-orange-100 shadow-card p-5">
                        <h3 className="font-poppins font-bold text-lg text-brand-dark mb-3">Delivery Areas</h3>
                        <div className="space-y-2 text-sm text-brand-muted">
                          <p className="flex items-center gap-2"><Truck size={15} className="text-primary" /> {vendor.address || 'Citywide delivery available'}</p>
                          <p className="flex items-center gap-2"><MapPin size={15} className="text-primary" /> Local neighborhoods served daily</p>
                        </div>
                      </div>

                      <div className="rounded-[1.8rem] bg-white border border-orange-100 shadow-card p-5">
                        <h3 className="font-poppins font-bold text-lg text-brand-dark mb-3">Contact</h3>
                        <div className="space-y-2 text-sm text-brand-muted">
                          {vendor.phone && <p className="flex items-center gap-2"><Phone size={15} className="text-primary" /> {vendor.phone}</p>}
                          {vendor.whatsapp && <p className="flex items-center gap-2"><MessageSquare size={15} className="text-primary" /> WhatsApp available</p>}
                          <p className="flex items-center gap-2"><Info size={15} className="text-primary" /> Fast replies during business hours</p>
                        </div>
                      </div>
                    </div>
                  </section>
            ...
            <aside className="space-y-4">
              <SidebarCard title="Suggested Vendors" subtitle="More premium kitchens to explore">
                {sidebarLoading ? (
                  <div className="space-y-3">
                    {Array.from({ length: 3 }).map((_, i) => <VendorSkeleton key={i} />)}
                  </div>
                ) : suggestedVendors.length === 0 ? (
                  <p className="text-sm text-brand-muted">No suggestions yet.</p>
                ) : (
                  <div className="space-y-4">
              ...
              <SidebarCard title="Trending Food" subtitle="What customers are ordering now">
                {sidebarLoading ? (
                  <div className="space-y-3">
                  ...
                  <div className="space-y-3">
                    {trendingFoods.slice(0, 4).map((food) => (
                      <Link key={food.id} to={`/products/${food.id}`} className="flex items-center gap-3 rounded-2xl bg-brand-bg p-2.5 hover:bg-primary/5 transition-colors">
                        <img
                          src={food.images?.[0] || 'https://images.unsplash.com/photo-1565299624946-b28f40a0ae38?w=120'}
                          alt={food.name}
                          className="w-14 h-14 rounded-2xl object-cover"
                        />
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-semibold text-brand-dark truncate">{food.name}</p>
                          <p className="text-xs text-brand-muted truncate">{food.vendor?.businessName}</p>
                        </div>
                        <span className="text-xs font-bold text-primary">{formatPrice(Number(food.discountPrice) || Number(food.price))}</span>
                      </Link>
                    ))}
                  </div>
                )}
              ...
              <SidebarCard title="Current Offers" subtitle="Limited-time storefront promotions">
                <div className="space-y-3">
                  {posts.filter((p) => p.type === 'promotion').slice(0, 2).map((post) => (
                    <div key={post.id} className="rounded-2xl bg-gradient-to-br from-primary to-orange-500 text-white p-4">
                      <p className="text-xs uppercase tracking-[0.14em] text-white/70 mb-1">Special Offer</p>
                      <p className="font-semibold">{post.product?.name || post.caption?.slice(0, 40) || 'Vendor deal'}</p>
                      <Link to={post.product ? `/products/${post.product.id}` : '#'} className="inline-flex items-center gap-1 text-sm font-semibold mt-3">
                        View deal <ArrowRight size={14} />
                      </Link>
                    </div>
                  ))}
                  {posts.filter((p) => p.type === 'promotion').length === 0 && (
                    <p className="text-sm text-brand-muted">No active offers right now.</p>
                  )}
                </div>
              </SidebarCard>
...
function SidebarCard({ title, subtitle, children }) {
  return (
    <div className="rounded-[1.8rem] bg-white border border-orange-100 shadow-card p-4">
      <div className="mb-3">
        <h3 className="font-poppins font-bold text-lg text-brand-dark">{title}</h3>
