...
async function seed() {
    try {
        ...
        // ── 1. Categories ──────────────────────────────────────────
        const catRepo = database_1.AppDataSource.getRepository(Category_1.Category);
        const catMap = {};
        ...
        if (!(await userRepo.findOne({ where: { email: adminEmail } }))) {
            await userRepo.save(userRepo.create({
                ...
                password: await hash("Admin@2024"), role: User_1.UserRole.ADMIN,
        ...
        for (const v of VENDORS_DATA) {
            ...
            if (!vendorUser) {
                vendorUser = await userRepo.save(userRepo.create({
                    ...
                    password: await hash(v.password), role: User_1.UserRole.VENDOR,
                    isActive: true,
                }));
            }
            ...
            if (!vendor) {
                vendor = await vendorRepo.save(vendorRepo.create({
                    ...
                    status: Vendor_1.VendorStatus.APPROVED, isOpen: true,
        ...
        for (const c of CUSTOMERS_DATA) {
            ...
            if (!user) {
                user = await userRepo.save(userRepo.create({
                    ...
                    password: await hash(c.password), role: User_1.UserRole.CUSTOMER,
                    isActive: true,
                }));
        ...
        const ORDER_STATUSES = [
            Order_1.OrderStatus.DELIVERED, Order_1.OrderStatus.DELIVERED, Order_1.OrderStatus.DELIVERED,
            Order_1.OrderStatus.CONFIRMED, Order_1.OrderStatus.PREPARING, Order_1.OrderStatus.PENDING,
        ];
        let ordersCreated = 0;
        for (let i = 0; i < 35; i++) {
            ...
            if (vendorProds.length === 0)
                continue;
            const selectedProds = [rand(vendorProds), rand(vendorProds)].filter((p, idx, arr) => arr.findIndex((x) => x.id === p.id) === idx);
            let subtotal = 0;
            ...
            if (savedOrder.status === Order_1.OrderStatus.DELIVERED) {
                for (const prod of selectedProds) {
                ...
                await vendorRepo.update(vendor.id, {
                    totalEarnings: () => `totalEarnings + ${savedOrder.total}`,
                });
            }
        }
